...
async function doFetch(path, accessToken, init) {
...
export async function renameConversation(accessToken, id, title) {
...
export async function archiveConversation(accessToken, id, archived) {
...
export async function streamMessage(accessToken, params, onChunk, onDone) {
        ...
        while (true) {
            ...
            while ((idx = buffer.indexOf('\n\n')) !== -1) {
                ...
                try {
                    ...
                    if (obj.type === 'chunk' && obj.content && onChunk)
                        onChunk(obj.content);
                    if (obj.type === 'done' && onDone)
                        onDone();
                }
                catch {
